...
public class BreakObject : MonoBehaviour
{
    ...
    public float ExplodeForce = 5.0f;
    ...
    public void DestroyMesh()
    {
        ...
        for (var c = 0; c < CutCascades; c++)
        {
            for (var i = 0; i < parts.Count; i++)
            {
                ...
                var plane = new Plane(UnityEngine.Random.onUnitSphere, new Vector3(UnityEngine.Random.Range(bounds.min.x + (bounds.max.x - bounds.min.x / 3), bounds.max.x - (bounds.max.x - bounds.min.x / 3)), UnityEngine.Random.Range(bounds.min.y + (bounds.max.y - bounds.min.y / 3), bounds.max.y - (bounds.max.y - bounds.min.y / 3)), UnityEngine.Random.Range(bounds.min.z + (bounds.max.z - bounds.min.z / 3), bounds.max.z - (bounds.max.z - bounds.min.z / 3))));
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(BreakObject original)
        {
            ...
            var meshDestroy = GameObject.AddComponent<BreakObject>();
            meshDestroy.CutCascades = original.CutCascades;
            meshDestroy.ExplodeForce = original.ExplodeForce;
            GameObject.GetComponent<MonoBehaviour>().StartCoroutine(GameObject.GetComponent<BreakObject>().DestroyWithDelay());
        }
    }
    IEnumerator DestroyWithDelay()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            mat.SetFloat("Vector1_DDCD963", val);
        }
